...
### Added
- **🕊️ The playground keeps pace.** The in-browser interpreter at `site/playground/index.html` now speaks the language's newest words: `TRY … CATCH … ENDTRY` (sheltering fragile works, binding the plain error message to `ERROR` or a name of your choosing, `RETURN` and `ASCEND` still passing straight through, errors in the `CATCH` body rising outward as they should) and `BREAK` / `CONTINUE` for loops (rejected outside a loop at parse time, and never allowed to cross a rite boundary, exactly like the engine). A new "Sheltered by Grace (TRY/CATCH)" sample boots the palette at seven creations; the site learn page gained a "Shelter a fragile work" lesson; new example `examples/try_catch_demo.god` runs green through `run`, `check`, and `fmt`
